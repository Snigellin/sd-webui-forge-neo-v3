...
from scripts.agent_chat import chat_stream
...
def on_ui_tabs():
    ...
    with gr.Blocks(
            ...
            with gr.Column(scale=3):
                ...
                with gr.Row():
                    image_aspect_ratio = gr.Dropdown(
                        label="📐 画面比例",
                        choices=["1:1", "9:16", "16:9", "参考图比例"],
                        value=cfg_init.get("image_aspect_ratio", "1:1"),
                        interactive=True,
                        scale=1,
                    )
                    ...
                    pause_btn = gr.Button("⏹️ 暂停思考", variant="stop", scale=1)
        ...
        def save_image_aspect_ratio(choice):
            cfg = load_config(resolve_local=False)
            choice = str(choice or "1:1").strip()
            if choice not in ("1:1", "9:16", "16:9", "参考图比例"):
                choice = "1:1"
            cfg["image_aspect_ratio"] = choice
            return "✅ 默认画面比例已保存为 " + choice if save_config(cfg) else "❌ 画面比例保存失败"
        ...
        image_aspect_ratio.change(fn=save_image_aspect_ratio, inputs=[image_aspect_ratio], outputs=[image_settings_status])
        ...
        # 终止当前生成器事件，立即停止继续思考/调用工具。
        pause_btn.click(fn=None, inputs=None, outputs=None, cancels=[send_event, submit_event])
