...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "shell_execute",
            "description": "执行系统命令（如 git clone、pip install、文件操作等）。用户授权了完整系统权限，可以安装 GitHub 插件、运行脚本、管理文件。Windows 上通过 PowerShell 执行。",
            "parameters": {
                "type": "object",
                "properties": {
                    "command": {"type": "string", "description": "要执行的命令，如 'git clone https://github.com/xxx/xxx.git'"},
                    "cwd": {"type": "string", "description": "工作目录（绝对路径），默认为整合包根目录", "default": ""},
    ...
    },
    {
        ...
        "function": {
            "name": "list_skills",
            "description": "列出智能体当前可用的所有 Skill（name/description/SKILL.md 路径）。当用户的问题可能匹配某个专业技能（如把插件结构、项目结构、工作链路渲染成图表）时，先调用此工具发现可用技能，再用 read_skill 读取完整指令后按指令行动。",
            "parameters": {
                "type": "object",
                "properties": {},
            },
    ...
    },
    {
        ...
        "function": {
            "name": "read_skill",
            "description": "读取指定技能的完整 SKILL.md 内容（含 frontmatter 元数据与正文指令），并严格按其步骤行动。技能名来自 list_skills 的返回。",
            "parameters": {
                "type": "object",
                "properties": {
                    "name": {"type": "string", "description": "技能名，如 'archify'"},
                    "max_chars": {"type": "integer", "description": "返回正文最大字符数", "default": 24000, "maximum": 999999},
                },
                "required": ["name"],
            },
        },
    },
    {
        ...
        "function": {
            ...
            "description": "校验 Archify 图表 JSON 规格（不产出 HTML）。用于创作/修复迭代：先读 schema 与示例写规格，再 validate，按回执诊断修复直到通过，最后用 generate_diagram 交付。",
            "parameters": {
                "type": "object",
                "properties": {
                    "type": {"type": "string", "description": "图类型", "enum": ["architecture", "workflow", "sequence", "dataflow", "lifecycle"]},
                    "spec": {"type": "string", "description": "JSON 规格字符串，或 .json 文件路径（相对路径基于整合包根目录）"},
                    "quality": {"type": "string", "description": "质量标准", "enum": ["standard", "showcase"], "default": "showcase"},
                },
                "required": ["type", "spec"],
            },
        },
    },
    {
        ...
        "function": {
            ...
            "description": "把 Archify JSON 规格渲染成交互式自包含 HTML 图表（内联 SVG，可缩放/深浅主题/导出图片）。用于可视化插件结构、项目结构、工作链路等。成功时返回 HTML 绝对路径，请用户用浏览器打开（聊天界面无法内嵌渲染 HTML）。规格建议先用 validate_diagram 校验通过。",
            "parameters": {
                "type": "object",
                "properties": {
                    "type": {"type": "string", "description": "图类型", "enum": ["architecture", "workflow", "sequence", "dataflow", "lifecycle"]},
                    "spec": {"type": "string", "description": "JSON 规格字符串，或 .json 文件路径（相对路径基于整合包根目录）"},
                    "quality": {"type": "string", "description": "质量标准", "enum": ["standard", "showcase"], "default": "showcase"},
                    "title": {"type": "string", "description": "可选，用于产物文件名，如 'agent-workflow'", "default": ""},
                },
                "required": ["type", "spec"],
            },
        },
    },
    {
        ...
        "function": {
            "name": "list_memories",
            "description": "检索用户的记忆库（过往对话中沉淀的事实/偏好/问答记忆）。用户想查看'你记住我什么'、'我之前问过什么'，或回答需要确认已有记忆时使用。返回记忆列表（id、类型、内容、更新时间）。",
            "parameters": {
                "type": "object",
                "properties": {
                    "keyword": {"type": "string", "description": "可选关键词，只返回匹配的记忆；为空则返回全部", "default": ""},
                },
            },
        },
    },
    {
        ...
        "function": {
            "name": "delete_memory",
            "description": "删除记忆库中的记忆。优先用 mem_id 精确删除单条（id 可来自 list_memories 结果或聊天中的记忆块）；也可用 keyword 删除所有匹配的记忆。仅在用户明确要求遗忘/删除某记忆时使用。",
            "parameters": {
                "type": "object",
                "properties": {
                    "mem_id": {"type": "string", "description": "要删除的记忆 id（如 'mem_079268'），精确删除", "default": ""},
                    "keyword": {"type": "string", "description": "要删除的记忆关键词，删除所有匹配项", "default": ""},
                },
            },
        },
    },
