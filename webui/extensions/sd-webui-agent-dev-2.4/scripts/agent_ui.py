...
def on_ui_tabs():
    ...
    with gr.Blocks(
        ...
        /* ===== 记忆检索可见块（聊天中的折叠记忆卡） ===== */
        #agent-chatbot details.agent-memory-block {
            margin: 6px 0 10px;
            max-width: min(72vw, 760px);
        ...
        #agent-chatbot details.agent-memory-block > summary {
            cursor: pointer;
            padding: 6px 10px;
            font-size: 12px;
            color: #38bdf8;
        ...
        #agent-chatbot details.agent-memory-block > summary::-webkit-details-marker {
            display: none;
        }
        #agent-chatbot details.agent-memory-block > summary::before {
            content: "▸ ";
        }
        #agent-chatbot details.agent-memory-block[open] > summary::before {
            content: "▾ ";
        }
        #agent-chatbot details.agent-memory-block .agent-memory-body {
            max-height: 200px;
            overflow-y: auto;
            padding: 2px 12px 10px;
            white-space: pre-wrap;
        ...
        #agent-chatbot details.agent-memory-block .agent-memory-body::-webkit-scrollbar {
            width: 6px;
        }
        #agent-chatbot details.agent-memory-block .agent-memory-body::-webkit-scrollbar-thumb {
            background: rgba(56, 189, 248, 0.4);
            border-radius: 3px;
        }
        #agent-chatbot details.agent-memory-block .agent-memory-item {
            margin: 4px 0;
        }
        #agent-chatbot details.agent-memory-block .agent-memory-answer {
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                with gr.Accordion("🧠 记忆库管理（记住你 / 重启续作）", open=False):
                    ...
                    memory_keyword = gr.Textbox(label="按关键词筛选/删除（留空查看全部）", value="")
                    with gr.Row():
                        memory_refresh_btn = gr.Button("🔄 刷新列表", size="sm", scale=1)
                        memory_delete_btn = gr.Button("🗑️ 删除匹配的记忆", size="sm", scale=1)
                        memory_clear_btn = gr.Button("🧹 清空全部", size="sm", scale=1)
                    memory_table = gr.Dataframe(
                        label="记忆库列表",
                        headers=["ID", "类型", "内容", "更新时间", "命中"],
                        datatype=["str", "str", "str", "str", "number"],
                        interactive=False,
                        wrap=True,
                    )
                    memory_status = gr.Textbox(show_label=False, interactive=False)
        ...
        # ===== 记忆库管理 =====
        def _memory_rows(keyword=""):
            """读取记忆库并转为 Dataframe 行，返回 (rows, 状态文本)。"""
            try:
                from scripts.agent_memory import _default as _mem
                mems = _mem.list_memories((keyword or "").strip() or None)
            except Exception as e:
                return [], f"❌ 读取失败: {e}"
            tag_map = {"fact": "事实", "preference": "偏好", "qa": "问答"}
            rows = []
            for m in mems:
                rows.append([
                    m.get("id", ""),
                    tag_map.get(m.get("category", ""), m.get("category", "记忆")),
                    m.get("content", ""),
                    (m.get("updated_at") or "")[:16].replace("T", " "),
                    m.get("hit_count", 0) or 0,
                ])
            kw = (keyword or "").strip()
            note = f'匹配 "{kw}"' if kw else "共"
            return rows, f"{note} {len(rows)} 条记忆"

        def memory_refresh(keyword=""):
            return _memory_rows(keyword)

        def memory_delete(keyword=""):
            kw = (keyword or "").strip()
            if not kw:
                rows, msg = _memory_rows(keyword)
                return rows, msg + " | ⚠️ 请先输入关键词再删除；全部删除请用「🧹 清空全部」"
            try:
                from scripts.agent_memory import _default as _mem
                n = _mem.delete_memory_by_keyword(kw)
            except Exception as e:
                rows, _ = _memory_rows(keyword)
                return rows, f"❌ 删除失败: {e}"
            rows, msg = _memory_rows()
            return rows, f"✅ 已删除 {n} 条记忆（关键词: {kw}） | " + msg

        def memory_clear():
            try:
                from scripts.agent_memory import _default as _mem
                n = _mem.clear_all_memories()
            except Exception as e:
                return [], f"❌ 清空失败: {e}"
            return [], f"🧹 已清空记忆库（共删除 {n} 条）。最近对话记录仍保留，用于重启续作。"

        def memory_toggle(enabled):
            try:
                cfg = load_config()
                cfg["memory_enabled"] = bool(enabled)
                save_config(cfg)
                return f"✅ 记忆功能已{'启用' if enabled else '关闭'}（立即生效，无需重启）"
            except Exception as e:
                return f"❌ 保存失败: {e}"
        ...
        memory_clear_btn.click(fn=memory_clear, outputs=[memory_table, memory_status])
        memory_enabled.change(fn=memory_toggle, inputs=[memory_enabled], outputs=[memory_status])
