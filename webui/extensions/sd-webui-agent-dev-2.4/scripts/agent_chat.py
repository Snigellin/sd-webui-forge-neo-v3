...
_MEMORY_BLOCK_RE = re.compile(r'<details[^>]*class="agent-memory-block"[^>]*>.*?</details>\s*', re.DOTALL)

_MEMORY_CATEGORY_TAG = {"fact": "事实", "preference": "偏好", "qa": "问答"}
...
def _strip_memory_block(text):
    """从历史消息中移除记忆可见块 HTML（仅 UI 展示，回放时剔除避免浪费 token）。"""
    if not isinstance(text, str) or "agent-memory-block" not in text:
        return text
    return _MEMORY_BLOCK_RE.sub("", text).strip()


def _strip_ui_blocks(text):
    """剔除所有 UI 专属块（思考块 + 记忆块），只保留正式内容。"""
    text = _strip_thinking_block(text)
    return _strip_memory_block(text)


def _memory_block(hits):
    """渲染本轮命中的记忆可见块（放在聊天记录中，方便用户查看/删除参考）。"""
    if not hits:
        return ""
    lines = [f'<details class="agent-memory-block"><summary>🧠 记忆检索：命中 {len(hits)} 条相关记忆（点击展开）</summary>',
             '<div class="agent-memory-body">']
    for h in hits[:3]:
        tag = _MEMORY_CATEGORY_TAG.get(h.get("category", ""), "记忆")
        content = (h.get("content") or "").strip()
        answer = (h.get("answer") or "").strip()
        lines.append(f"<div class=\"agent-memory-item\"><b>[{tag}]</b> {content}")
        if answer:
            lines.append(f'<div class="agent-memory-answer">📌 记忆回答：{answer}</div>')
        lines.append("</div>")
...
def _audio_duration_seconds(path, ext):
    """尽力检测音频时长（秒）。WAV 用标准库 wave；其余格式依赖 mutagen（可选）。失败返回 None。"""
    try:
        if ext == ".wav":
...
def chat_stream(history, uploaded_image=None, uploaded_video=None, attachments=None):
    ...
    _sys_prompt = _get_system_prompt(cfg.get("model", ""))
    mem_hits = []
    if cfg.get("memory_enabled"):
        try:
            from scripts.agent_memory import prepare_memory
            _mem = prepare_memory(user_message, cfg, source="webui", include_recent_context=True)
            if _mem.get("prompt_block"):
                _sys_prompt += "\n\n" + _mem["prompt_block"]
            mem_hits = _mem.get("hits") or []
        except Exception as _e:
            print(f"[Agent][Memory] 注入失败: {_e}")
    ...
    for h in history:
        ...
        if role == "system":
            ...
            _flush_pending_user()
            if isinstance(content, str):
                # 剔除 UI 用的思考块/记忆块，只把正式回答回传给模型
                _clean = _strip_ui_blocks(content)
                if _clean:
    ...
    # 记忆可见块：本轮命中记忆时作为独立块放进聊天记录（回放时会被剔除，不占 token）
    new_history = list(history)
    if mem_hits:
        new_history.append({"role": "assistant", "content": _memory_block(mem_hits)})
    ...
    try:
        ...
        for iteration in range(cfg["max_tool_iterations"]):
            ...
            if tool_calls_accumulator:
                ...
                for tc in assistant_tool_calls:
                    ...
                    # 只要本轮有上传图片，就必须走编辑接口；api_image_generate
                    # 不接收参考图，若让 LLM 选到它会把图片静默丢掉。
                    if uploaded_image_list and tool_name == "api_image_generate":
                        tool_name = "api_image_edit"
                        tool_args = {
                            "model": _configured_image_model(),
                            "instruction": tool_args.get("prompt") or user_message,
                        }
                        print("[Agent] 检测到上传参考图，已将 api_image_generate 强制切换为 api_image_edit")
        ...
        # 记录最近一轮（跨重启续作：重启后再问可以接着上次继续）
        try:
            from scripts.agent_memory import record_turn
            record_turn(user_message, answer_text, cfg, source="webui")
        except Exception:
            pass

        # 记忆抽取（后台线程，不阻塞）
        try:
            from scripts.agent_memory import maybe_extract_and_store
            maybe_extract_and_store(user_message, answer_text, cfg, source="webui")
        except Exception:
            pass
